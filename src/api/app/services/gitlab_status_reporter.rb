class GitlabStatusReporter < SCMExceptionHandler
  ...
  def call
    ...
    if @workflow_run.present?
      ...
      RabbitmqBus.send_to_bus('metrics', "scm_status_report,status=success,scm=#{@worklow_run.scm_vendor} value=1")
    end
  rescue Gitlab::Error::Error, OpenSSL::SSL::SSLError => e
    rescue_with_handler(e) || raise(e)
    RabbitmqBus.send_to_bus('metrics', "scm_status_report,status=fail,scm=#{@worklow_run.scm_vendor},exception=#{e.class} value=1") if @workflow_run.present?
  end
